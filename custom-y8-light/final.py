...
def detect_and_send():
    while True:
        ...
        if detected_objects:
            command = ",".join(detected_objects)  # Convert list to a comma-separated string
            if command == "green-lights":
                esp_serial.write(b'1')  # Send over serial
                print(f"Sent to ESP32: {command}")

            elif command == "red-lights":
                esp_serial.write(b'0')  # Send over serial
                print(f"Sent to ESP32: {command}")

            elif command == "yellow-lights":
                esp_serial.write(b'6')  # Send over serial
                print(f"Sent to ESP32: {command}")
